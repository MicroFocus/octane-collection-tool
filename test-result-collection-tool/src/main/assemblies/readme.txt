...
If the configuration file is named 'config.properties' and is in the same
...
If --build-context-server-id is omitted, the tool deduces it from the combination
of job id + build id + workspace, but pushes code coverage to all instances of the pipeline across the shared space.
...
file. Password is entered directly on the command line, and test fields and run environment tags are assigned to
...
file, which is placed in the same directory as this tool. Result file appears in internal XML format (-i option).

    java -jar test-result-collection-tool.jar -i publicApi.xml

4.  Send test results with build context to update pipeline builds and suite runs.
...
8.  Push code coverage reports and test results without providing the -- build-context-server-id parameter
    (deduced from jobId and buildId).
